...
import gestionstagiaire_marsamaroc.Assistante_DRPTM2;
import gestionstagiaire_marsamaroc.Assistante_DA;
import gestionstagiaire_marsamaroc.Assistante_DAIO;
import gestionstagiaire_marsamaroc.Assistante_DAJ;
import gestionstagiaire_marsamaroc.Assistante_DDS;
import gestionstagiaire_marsamaroc.Assistante_DF;
import gestionstagiaire_marsamaroc.Assistante_DPO;
import gestionstagiaire_marsamaroc.Assistante_DRH;
import gestionstagiaire_marsamaroc.Assistante_DSI;
...
public class AuthentificationAssistante extends javax.swing.JFrame {
    
    Connection conn=null;
    Statement st=null;
    ResultSet rs=null;

    public AuthentificationAssistante() {
        initComponents();
    }
    ...
    private void LoginButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_LoginButtonActionPerformed
        ...
        try{
            ...
            rs=st.executeQuery("select Login,Password from utilisateur where role='assistante'");
            boolean log=false;
            int i=1;
            while(rs.next()){
            ...
            if(log==true){
                if(i==1){
                    //systemExit();
                    Assistante_DRPTM2 Assist1 =new Assistante_DRPTM2();
                    Assist1.setVisible(true);
                    this.dispose();
                }
                if(i==2){
                    //systemExit();
                    Assistante_DF Assist2 =new Assistante_DF();
                    Assist2.setVisible(true);
                    this.dispose();
                }
                if(i==3){
                    //systemExit();
                    Assistante_DA Assist3 =new Assistante_DA();
                    Assist3.setVisible(true);
                    this.dispose();
                }
                if(i==4){
                    //systemExit();
                    Assistante_DDS Assist4 =new Assistante_DDS();
                    Assist4.setVisible(true);
                    this.dispose();
                }
                if(i==5){
                    //systemExit();
                    Assistante_DPO Assist5 =new Assistante_DPO();
                    Assist5.setVisible(true);
                    this.dispose();
                }
                if(i==6){
                    //systemExit();
                    Assistante_DAIO Assist6 =new Assistante_DAIO();
                    Assist6.setVisible(true);
                    this.dispose();
                }
                if(i==7){
                    //systemExit();
                    Assistante_DRH Assist7 =new Assistante_DRH();
                    Assist7.setVisible(true);
                    this.dispose();
                }
                if(i==8){
                   // systemExit();
                    Assistante_DSI Assist8 =new Assistante_DSI();
                    Assist8.setVisible(true);
                    this.dispose();
                }
                if(i==9){
                    //systemExit();
                    Assistante_DAJ Assist9 =new Assistante_DAJ();
                    Assist9.setVisible(true);
                    this.dispose();
                }  
            }
            else{
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AuthentificationAssistante().setVisible(true);
            }
        });
    }
